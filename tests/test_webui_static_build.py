...
class WebUIStaticBuildTests(WebUIStaticTestCase):
    ...
    def test_server_launchers_keep_workspace_assets_and_container_logs(self) -> None:
        dockerfile = Path("Dockerfile.server").read_text(encoding="utf-8")
        compose = Path("compose.server.yml").read_text(encoding="utf-8")

        self.assertIn("COPY codex_image ./codex_image", dockerfile)
        self.assertNotIn("rm -rf codex_image/webui", dockerfile)
        self.assertIn("PYTHONUNBUFFERED=1", dockerfile)
        self.assertIn("codex_image.server.web:app", dockerfile)
