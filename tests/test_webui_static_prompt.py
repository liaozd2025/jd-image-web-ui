...
class WebUIStaticPromptTests(WebUIStaticTestCase):
    ...
    def test_prompt_fidelity_control_supports_original_mode_and_is_submitted(self) -> None:
        ...
        self.assertIn("usesLegacyWorkspaceControls(state.selectedModelId, selectedModel?.family_id)", script)
    ...
    def test_prompt_template_library_has_compact_entry_and_drawer(self) -> None:
        ...
        self.assertRegex(styles, r"\.prompt-template-row\s*\{[^}]*grid-template-columns:\s*minmax\(0,\s*1fr\)\s+minmax\(240px,\s*300px\)\s+var\(--prompt-action-column-width\)")
    ...
    def test_prompt_editor_supports_prompt_snippet_chips(self) -> None:
        ...
        self.assertIn("输入 ~ 或 ～ 可调用提示词模板或片段", html)
        ...
        self.assertIn("promptSnippetSuggestOptions", script)
        self.assertIn("promptTemplatesForQuery", script)
        self.assertIn("insertPromptTemplate", script)
        self.assertIn('querySelector("[data-prompt-template-id], [data-prompt-snippet-id]")', script)
        self.assertIn("data-prompt-template-id", script)
...
"""
    ...
    def test_main_model_control_is_persisted_and_submitted(self) -> None:
        ...
        self.assertIn("usesLegacyMainModelControl(state.selectedModelId)", script)
    ...
    def test_api_direct_mode_hides_non_applicable_main_model_but_keeps_prompt_fidelity(self) -> None:
        ...
        self.assertIn("return !state.generationCatalog || usesLegacyWorkspaceControls(state.selectedModelId, model?.family_id);", script)
