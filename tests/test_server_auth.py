...
class ServerAuthenticationFlowTests(unittest.TestCase):
    def test_browser_login_requires_one_time_password_change_then_logout_revokes_session(self) -> None:
        ...
        self.assertIn("九典制药", login_page.text)
        self.assertIn("图片内容生产平台", login_page.text)
        self.assertIn('src="/auth-static/jiudian-pharma-logo.png?v=jiudian-brand-1"', login_page.text)
        self.assertIn('width="34" height="40"', login_page.text)
        self.assertTrue(Path("codex_image/server/static/jiudian-pharma-logo.png").exists())
        ...
        self.assertIn('class="layout-container"', home.text)
        self.assertIn('id="sidebar"', home.text)
        self.assertIn('/static/app.js', home.text)
        self.assertNotIn('id="server-home"', home.text)
        self.assertEqual(admin_home.status_code, 200)
        self.assertIn('class="system-settings-shell hidden"', admin_home.text)
        self.assertIn('data-system-settings-tab="users"', admin_home.text)
        self.assertIn("settingsTab=users", str(admin_home.url))
        ...
        self.assertEqual(current_user.json()["csrf_token"], changed.json()["csrf_token"])
        self.assertTrue(current_user.json()["session"]["session_id"])
        self.assertEqual(current_user.json()["session"]["user_agent"], "testclient")
        self.assertTrue(current_user.json()["session"]["current"])
