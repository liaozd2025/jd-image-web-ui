...
class WebUIStaticI18nTests(WebUIStaticTestCase):
    ...
    def test_language_bootstrap_detects_browser_language_and_settings_select_replaces_top_nav(self) -> None:
        ...
        self.assertIn('const LOCALE_STORAGE_KEY = "codex-image-locale-preference";', bootstrap)
        self.assertIn("function detectPreferredLocale", bootstrap)
        self.assertIn("async function readStoredLocalePreference", bootstrap)
        self.assertIn("function persistLocalePreference", bootstrap)
        self.assertIn('fetch("/api/settings")', bootstrap)
        self.assertIn('body: JSON.stringify({ locale: currentLocale })', bootstrap)
        self.assertIn("navigator.languages", bootstrap)
        self.assertIn('const validLocales = new Set(["zh-CN", "zh-TW", "zh-HK", "ja", "ko", "en", "es", "pt", "fr", "de", "ru", "it", "hi"]);', bootstrap)
        self.assertRegex(bootstrap, r"document\.documentElement\.lang = currentLocale;")
        ...
        self.assertIn('language.startsWith("ru")', bootstrap)
        self.assertIn('language.startsWith("it")', bootstrap)
        self.assertIn('language.startsWith("hi")', bootstrap)
