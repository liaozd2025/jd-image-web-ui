...
class WebUIStaticI18nTests(WebUIStaticTestCase):
    ...
    def test_language_bootstrap_detects_browser_language_and_settings_select_replaces_top_nav(self) -> None:
        ...
        self.assertIn('src="/static/workspace-preload.js?v=server-1"', html)
        self.assertIn('const LOCALE_STORAGE_KEY = "codex-image-locale-preference";', locale_sources)
        self.assertIn("function detectPreferredLocale", locale_sources)
        self.assertIn("async function readStoredLocalePreference", locale_sources)
        self.assertIn("function persistLocalePreference", locale_sources)
        self.assertIn('fetch("/api/settings")', locale_sources)
        self.assertIn('body: JSON.stringify({ locale: currentLocale })', locale_sources)
        self.assertIn("navigator.languages", locale_sources)
        self.assertIn('const validLocales = new Set(["zh-CN", "zh-TW", "zh-HK", "ja", "ko", "en", "es", "pt", "fr", "de", "ru", "it", "hi"]);', preload)
        self.assertRegex(locale_sources, r"document\.documentElement\.lang = currentLocale;")
        ...
        self.assertIn('language.startsWith("ru")', locale_sources)
        self.assertIn('language.startsWith("it")', locale_sources)
        self.assertIn('language.startsWith("hi")', locale_sources)
