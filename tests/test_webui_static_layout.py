...
class WebUIStaticLayoutTests(WebUIStaticTestCase):
    ...
    def test_server_account_controls_are_available_in_top_nav(self) -> None:
        html = Path("codex_image/webui/static/index.html").read_text(encoding="utf-8")
        script = self._frontend_script_source()
        ...
        self.assertNotIn('id="authSourceDetail"', html)
        ...
        self.assertIn('fetch("/api/auth/me")', account_source)
        self.assertIn('fetch("/api/auth/logout"', account_source)
        ...
        self.assertRegex(styles, r"\.server-account\s*\{[^}]*display:\s*inline-flex")
        self.assertRegex(styles, r"\.server-account\s*\{[^}]*align-items:\s*center")
        self.assertRegex(styles, r"\.server-account-name\s*\{[^}]*text-overflow:\s*ellipsis")
        self.assertRegex(styles, r"\.server-account-action\s*\{[^}]*white-space:\s*nowrap")
