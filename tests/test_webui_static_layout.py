...
class WebUIStaticLayoutTests(WebUIStaticTestCase):
    ...
    def test_recent_reference_assets_are_rendered_and_submitted(self) -> None:
        ...
        self.assertIn('/static/app.js?v=runtime-237', html)
        self.assertIn('/static/styles.css?v=runtime-237', html)
    ...
    def test_api_settings_feature_has_typescript_source_contract(self) -> None:
        ...
        for function_name in [
            ...
            self._assert_bootstrap_proxy(legacy_source, function_name)
    def test_account_quota_feature_has_typescript_source_contract(self) -> None:
        account_quota_source = self._account_quota_source()
        legacy_source = self._bootstrap_source()
        main_source = Path("codex_image/webui/frontend/src/main.ts").read_text(encoding="utf-8")
        ...
        self.assertLess(main_source.index('import { initAccountQuotaFeature } from "./account-quota"'), main_source.index('import { initPromptFeature } from "./prompt"'))
        self.assertLess(main_source.index("initApiSettingsFeature()"), main_source.index("initAccountQuotaFeature()"))
        self.assertLess(main_source.index("initAccountQuotaFeature()"), main_source.index("initPromptFeature()"))
        self.assertIn("export function initAccountQuotaFeature", account_quota_source)
        self.assertIn("async function refreshAccountQuota(refresh", account_quota_source)
        self.assertIn("function renderAccountQuota()", account_quota_source)
        self.assertIn("function accountQuotaLimitRowsHtml(item", account_quota_source)
        self.assertIn("function toggleAccountQueueEnabled(accountKey", account_quota_source)
        self.assertIn("function openAccountQuotaDrawer()", account_quota_source)
        self.assertIn("function closeAccountQuotaDrawer()", account_quota_source)
        self.assertIn("Object.assign(getLegacyBridge().methods", account_quota_source)
        for function_name in [
            "refreshAccountQuota",
            "renderAccountQuota",
    ...
    def test_account_quota_drawer_is_available(self) -> None:
        html = Path("codex_image/webui/static/index.html").read_text(encoding="utf-8")
        script = self._frontend_script_source()
        styles = Path("codex_image/webui/static/styles.css").read_text(encoding="utf-8")

        self.assertIn('id="accountQuotaButton"', html)
        self.assertIn('id="accountQuotaDrawer"', html)
        self.assertIn('id="accountQuotaList"', html)
        self.assertIn('id="accountQuotaDrawerClose"', html)
        self.assertIn('aria-label="关闭账号额度面板"', html)
        self.assertIn('<svg class="drawer-close-icon"', html)
        self.assertIn("accountQuota: { items: [], summary: {} }", script)
        self.assertIn('fetch(`/api/accounts${refresh ? "?refresh=true" : ""}`)', script)
        self.assertIn("function renderAccountQuota()", script)
        self.assertIn("openAccountQuotaDrawer", script)
        self.assertIn("closeAccountQuotaDrawer();", script)
        self.assertIn("function toggleAccountQueueEnabled", script)
        self.assertIn("summary.usable_channel_count", script)
        self.assertIn("function accountQuotaLimitRowsHtml", script)
        self.assertIn("function accountQuotaPercentValue", script)
        self.assertIn('percent === null || percent === undefined || percent === ""', script)
        self.assertIn("codex_5h_percent", script)
        self.assertIn("codex_week_percent", script)
        self.assertIn("account-quota-limit-row", script)
        self.assertIn("account-quota-limit-bar", script)
        self.assertIn("account-quota-switch-track", script)
        self.assertIn("aria-label=\"参与轮询", script)
        self.assertIn("data-account-manual-disabled-key", script)
        self.assertIn('fetch(`/api/accounts/${encodeURIComponent(accountKey)}`', script)
        self.assertIn('"manual_disabled"', script)
        self.assertIn("参与轮询", script)
        self.assertNotIn("剩余额度", script)
        self.assertNotIn("<span>参与轮询</span>", script)
        self.assertIn("已停用", script)
        self.assertRegex(styles, r"\.account-quota-list\s*\{[^}]*display:\s*flex")
        self.assertRegex(styles, r"\.account-quota-card\s*\{[^}]*border:")
        self.assertRegex(styles, r"\.account-quota-limit-row\s*\{[^}]*display:\s*grid")
        self.assertRegex(styles, r"\.account-quota-limit-bar\s*\{[^}]*height:\s*7px")
        self.assertRegex(styles, r"\.account-quota-limit-bar\s+span\s*\{[^}]*width:\s*var\(--quota-percent\)")
        self.assertRegex(styles, r"\.account-quota-toggle\s*\{[^}]*width:\s*48px")
        self.assertRegex(styles, r"\.account-quota-toggle\s*\{[^}]*height:\s*28px")
        self.assertRegex(styles, r"\.account-quota-toggle\s*\{[^}]*border-radius:\s*999px")
        self.assertRegex(styles, r"\.account-quota-toggle::after\s*\{[^}]*width:\s*22px")
        self.assertRegex(styles, r"\.account-quota-toggle\.active::after\s*\{[^}]*transform:\s*translateX\(20px\)")
    ...
    def test_auth_source_switcher_is_available_in_top_nav(self) -> None:
        ...
        self.assertIn('data-auth-source="auto"', html)
        self.assertIn('data-auth-source="cockpit"', html)
        ...
        self.assertLess(html.index('id="accountQuotaButton"'), html.index('id="queueButton"'))
        ...
        self.assertIn('if (source === "api" && currentAuthSource() === "api")', script)
        ...
        self.assertIn('title="已选中时再次点击或双击打开 API 设置"', html)
        self.assertIn('class="auth-source-settings-icon"', html)
        ...
        self.assertIn('els.accountQuotaButton?.classList.toggle("reserved-hidden", selected === "api")', script)
        self.assertIn('els.accountQuotaButton?.setAttribute("aria-hidden", selected === "api" ? "true" : "false")', script)
        self.assertIn('els.accountQuotaButton?.setAttribute("aria-expanded", "false")', script)
        self.assertIn('els.accountQuotaButton?.setAttribute("tabindex", "-1")', script)
        self.assertIn('els.accountQuotaButton?.removeAttribute("tabindex")', script)
        ...
        self.assertRegex(styles, r"\.account-quota-button\s*\{[^}]*min-width:\s*90px")
        self.assertRegex(styles, r"\.account-quota-button\.reserved-hidden\s*\{[^}]*visibility:\s*hidden")
        self.assertRegex(styles, r"\.account-quota-button\.reserved-hidden\s*\{[^}]*pointer-events:\s*none")
        self.assertRegex(styles, r"\.auth-source-api-button\s*\{[^}]*gap:\s*4px")
        self.assertRegex(styles, r"\.auth-source-settings-icon\s*\{[^}]*width:\s*12px")
        ...
        self.assertIn('return `自动 → ${effective}${provider ? ` · ${provider}` : ""} · ${mode}`;', auth_source)
        self.assertIn('return `${effective}${provider ? ` · ${provider}` : ""} · ${mode}`;', auth_source)
        self.assertIn('return `${selected}${provider ? ` · ${provider}` : ""} 不可用`;', auth_source)
    ...
    def test_api_source_switcher_and_settings_modal_exist(self) -> None:
        ...
        self.assertIn("openApiSettingsModal", script)
        self.assertIn("handleAuthSourceDoubleClick", script)
        self.assertIn('authSourceGroup?.addEventListener("dblclick", (event) => call(methods, "handleAuthSourceDoubleClick", event))', script)
    ...
    def test_primary_panel_close_buttons_use_consistent_x_button(self) -> None:
        ...
        expected_buttons = {
            "accountQuotaDrawerClose": "关闭账号额度面板",
