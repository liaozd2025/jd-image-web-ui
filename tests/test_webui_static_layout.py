...
class WebUIStaticLayoutTests(WebUIStaticTestCase):
    ...
    def test_gallery_feature_has_typescript_source_contract(self) -> None:
        ...
        self.assertIn("async function openGallery(scope", gallery_source)
        ...
        self.assertIn('openGallery("personal")', gallery_source)
        self.assertIn('openGallery("shared")', gallery_source)
        self.assertIn('els.galleryPersonalManageButton?.addEventListener("click"', gallery_source)
        self.assertIn('els.gallerySharedManageButton?.addEventListener("click"', gallery_source)
    ...
    def test_form_controls_feature_has_typescript_source_contract(self) -> None:
        ...
        generation_route_source = Path("codex_image/server/workspace_api.py").read_text(encoding="utf-8")
        ...
        self.assertIn("appendCanonicalGenerationFields(form, selection)", task_submit_source)
        self.assertIn("appendServerCompatibleGenerationFields(form, selection, currentTaskParams())", task_submit_source)
        ...
        self.assertIn('"canvas.resolution"', generation_route_source)
        self.assertIn('"canvas.aspect_ratio"', generation_route_source)
        self.assertIn('form.get("orientation")', generation_route_source)
    ...
    def test_sidebar_brand_uses_jiudian_identity(self) -> None:
        html = Path("codex_image/webui/static/index.html").read_text(encoding="utf-8")
        styles = Path("codex_image/webui/static/styles.css").read_text(encoding="utf-8")

        self.assertIn("<title>九典制药图片内容生产平台</title>", html)
        self.assertIn('<link rel="icon" type="image/png" href="/static/brand/pwa-icon-192.png" />', html)
        ...
        self.assertIn('src="/static/brand/jiudian-pharma-logo.png"', html)
        self.assertIn('<div class="brand-name">九典制药</div>', html)
        self.assertIn('<div class="brand-subtitle">图片内容生产平台</div>', html)
        self.assertIn('aria-label="九典制药图片内容生产平台"', html)
        self.assertNotIn("GPT-image-2 Studio", html)
        logo_path = Path("codex_image/webui/static/brand/jiudian-pharma-logo.png")
        ...
        self.assertRegex(styles, r"\.brand-logo\s*\{[^}]*object-fit:\s*contain")
        self.assertRegex(styles, r"\.brand-name\s*\{[^}]*font-size:\s*17px")
        self.assertRegex(styles, r"\.brand-subtitle\s*\{[^}]*font-weight:\s*700")
        self.assertRegex(styles, r"\.brand-subtitle\s*\{[^}]*letter-spacing:\s*0\.04em")
    ...
    def test_webui_brand_name_is_consistent_across_launch_surfaces(self) -> None:
        app_source = Path("codex_image/server/app.py").read_text(encoding="utf-8")
        login_html = Path("codex_image/server/static/login.html").read_text(encoding="utf-8")
        workspace_html = Path("codex_image/webui/static/index.html").read_text(encoding="utf-8")

        self.assertIn('FastAPI(title="九典制药图片内容生产平台"', app_source)
        for source in (app_source, login_html, workspace_html):
            self.assertIn("九典制药", source)
            self.assertIn("图片内容生产平台", source)
    ...
    def test_image_input_uses_quick_gallery_dock(self) -> None:
        ...
        self.assertRegex(html, r'id="galleryPersonalManageButton"[\s\S]*<span[^>]*>管理个人图库</span>')
        self.assertRegex(html, r'id="gallerySharedManageButton"[\s\S]*<span[^>]*>管理共享图库</span>')
        ...
        self.assertIn("galleryPersonalManageButton: document.querySelector(\"#galleryPersonalManageButton\")", script)
        self.assertIn("gallerySharedManageButton: document.querySelector(\"#gallerySharedManageButton\")", script)
        self.assertIn("renderQuickGalleryDock", script)
        self.assertIn("sortGalleryItems", script)
        self.assertIn('localeCompare(String(right.name || ""), "zh-CN", { numeric: true, sensitivity: "base" })', script)
        ...
        self.assertIn('openGallery("personal")', script)
        self.assertIn('openGallery("shared")', script)
        ...
        self.assertRegex(styles, r"\.gallery-manage-panel\s*\{[^}]*display:\s*grid")
        self.assertRegex(styles, r"\.gallery-manage-panel\s*\{[^}]*grid-template-columns:\s*repeat\(2,\s*minmax\(0,\s*1fr\)\)")
        self.assertRegex(styles, r"\.gallery-manage-panel\s*\{[^}]*width:\s*100%")
        self.assertNotRegex(styles, r"\.gallery-manage-panel\s*\{[^}]*border:")
        self.assertRegex(styles, r"\.gallery-manage-panel\s+\.resource-manage-button\s*\{[^}]*width:\s*auto")
    ...
    def test_gallery_categories_are_managed_from_server_in_management_sheet(self) -> None:
        ...
        self.assertIn('isSharedGallery() ? "/api/shared-gallery/categories" : "/api/gallery/categories"', script)
        ...
        self.assertIn('categoryEndpoint("/reorder")', script)
        ...
        self.assertIn('shared ? "/api/shared-gallery/items/reorder" : "/api/gallery/reorder"', script)
    ...
    def test_gallery_item_edits_use_inline_popover(self) -> None:
        ...
        self.assertIn('event.target.closest?.("[data-gallery-use],[data-gallery-restore],[data-gallery-rename],[data-gallery-replace],[data-gallery-move],[data-gallery-note],[data-gallery-delete]")', script)
    ...
    def test_server_account_controls_are_available_in_sidebar_footer(self) -> None:
        html = Path("codex_image/webui/static/index.html").read_text(encoding="utf-8")
        account_source = Path("codex_image/webui/frontend/src/server-account.ts").read_text(encoding="utf-8")
        styles = Path("codex_image/webui/static/styles.css").read_text(encoding="utf-8")

        self.assertIn('id="serverAccount" class="sidebar-account hidden"', html)
        ...
        self.assertIn('document.documentElement.dataset.userRole = context.user.role', account_source)
        self.assertRegex(styles, r"\.sidebar-account\s*\{[^}]*position:\s*relative")
        self.assertRegex(styles, r"\.sidebar-account-menu\s*\{[^}]*position:\s*absolute")
    def test_top_navigation_regression_contract(self) -> None:
        html = Path("codex_image/webui/static/index.html").read_text(encoding="utf-8")
        script = self._frontend_script_source()
        styles = Path("codex_image/webui/static/styles.css").read_text(encoding="utf-8")
    ...
    def test_unified_role_aware_system_settings_shell_exists(self) -> None:
        html = Path("codex_image/webui/static/index.html").read_text(encoding="utf-8")
        script = self._frontend_script_source()
        styles = Path("codex_image/webui/static/styles.css").read_text(encoding="utf-8")

        self.assertIn('class="system-settings-shell hidden"', html)
        self.assertIn('class="system-settings-sidebar"', html)
        self.assertIn('class="system-settings-main"', html)
        self.assertIn('data-system-settings-tab="account"', html)
        self.assertIn('data-system-settings-tab="users"', html)
        self.assertNotIn('data-system-settings-tab="api"', html)
        self.assertEqual(html.count('data-system-settings-tab="catalog"'), 1)
        self.assertEqual(html.count('data-system-settings-panel="catalog"'), 1)
        self.assertIn('data-admin-only', html)
        self.assertIn('id="systemSettingsSearch"', html)
        self.assertIn('id="systemSettingsApiPanel"', html)
        self.assertIn('id="apiBaseUrl"', html)
        self.assertIn('id="toggleApiProviderStatusButton"', html)
        self.assertIn('id="generationProviderSettingsButton" class="auth-source-settings-button hidden"', html)
        self.assertNotIn('id="settingsCatalogForm"', html)
        self.assertNotIn('id="settingsCatalogList"', html)
        self.assertNotIn('id="settingsDepartmentProviderList"', html)
        self.assertIn('const ADMIN_TABS', script)
        personal_tabs = script.split("const PERSONAL_TABS", 1)[1].split(";", 1)[0]
        self.assertNotIn('"api"', personal_tabs)
        self.assertIn("confirmToggleApiProviderStatus", script)
        self.assertIn("/api/admin/provider-catalog/", script)
        self.assertIn('codex-image-settings-tab-change', script)
        self.assertIn('const LAST_TAB_KEY', script)
        self.assertIn('window.history.replaceState', script)
        self.assertIn('window.history.pushState', script)
        self.assertIn('window.addEventListener("popstate", restoreSettingsFromHistory)', script)
        self.assertNotIn("window.history.forward()", script)
        self.assertIn('updateSettingsUrl(true, "replace")', script)
        self.assertIn("const dirtyOwners = new Set<HTMLElement>()", script)
        self.assertIn("dirtyOwners.delete(owner)", script)
        self.assertIn("replacePreservingDynamicDrafts", script)
        self.assertIn("settingsDraftKey", script)
        self.assertRegex(styles, r"\.system-settings-shell\s*\{[^}]*grid-template-columns:\s*310px")
    def test_api_provider_settings_regression_contract(self) -> None:
        ...
        for element_id in [
            ...
            "apiProviderBindings",
            "apiModelValidationSection",
            "apiModelList",
        ]:
            self.assertIn(f'id="{element_id}"', html)
            self.assertIn(f'document.querySelector("#{element_id}")', elements_source)
        self.assertLess(html.index('id="apiBaseUrl"'), html.index('id="apiKey"'))
        self.assertLess(html.index('id="apiKey"'), html.index('id="apiProviderBindings"'))
        self.assertLess(html.index('id="apiProviderBindings"'), html.index('id="apiModelValidationSection"'))
        ...
        self.assertIn('"X-CSRF-Token": getCsrfToken()', provider_source)
        self.assertIn("provider.read_only", provider_source)
        self.assertIn("provider.catalog_fields_read_only", provider_source)
        self.assertIn("generation_model_id", server_source)
        self.assertIn("canonical_model_id", server_source)
        self.assertIn("protocol_profile", server_source)

        self.assertIn(".provider-bindings-editor", styles)
        self.assertIn(".provider-bindings-list", styles)
        self.assertIn(".api-model-validation-section", styles)
        self.assertIn(".api-model-validation-row", styles)
        self.assertIn(".api-key-input-wrap", styles)
    ...
    def test_high_impact_settings_actions_require_confirmation(self) -> None:
        settings_script = Path("codex_image/webui/frontend/src/server-settings.ts").read_text(encoding="utf-8")
    ...
    def test_system_settings_uses_real_usage_instead_of_server_paths(self) -> None:
        ...
        self.assertIn('data-system-settings-tab="usage"', html)
        self.assertIn('id="settingsUsageSummary"', html)
        self.assertNotIn('id="settingsInputRoot"', html)
        self.assertNotIn('id="settingsOutputRoot"', html)
        self.assertNotIn('id="settingsGalleryRoot"', html)
        self.assertNotIn('id="settingsSourceDataRoot"', html)
        self.assertIn('api("/api/assets/quota")', settings_source)
        self.assertIn('api("/api/quotas/department")', settings_source)
        self.assertIn('id="settingsSchedulerBlocked"', html)
    ...
    def test_theme_mode_markup_and_bootstrap_exist(self) -> None:
        html = Path("codex_image/webui/static/index.html").read_text(encoding="utf-8")
        preload = Path("codex_image/webui/static/workspace-preload.js").read_text(encoding="utf-8")

        self.assertIn('<script src="/static/workspace-preload.js?v=server-1"></script>', html)
        self.assertIn('const THEME_STORAGE_KEY = "codex-image-theme-preference";', preload)
        self.assertRegex(preload, r"document\.documentElement\.dataset\.theme =")
