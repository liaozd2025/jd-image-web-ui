...
class WebUIStaticTaskTests(WebUIStaticTestCase):
    ...
    def test_history_page_static_contract_exists(self) -> None:
        ...
        self.assertIn('<link rel="icon" type="image/png" href="/static/brand/pwa-icon-192.png" />', history_html)
        ...
        self.assertIn('/static/history.js?v=history-75', history_html)
    ...
    def test_queue_frontend_polling_and_controls_exist(self) -> None:
        queue_routes = Path("codex_image/server/workspace_api.py").read_text(encoding="utf-8")
        events_source = queue_routes
        ...
        self.assertIn("async def task_events(request: Request)", queue_routes)
        self.assertIn("request.is_disconnected()", queue_routes)
        self.assertIn("StreamingResponse(", queue_routes)
        self.assertIn('"type": "snapshot"', events_source)
        self.assertIn('"tasks":', events_source)
        self.assertIn('"queue":', events_source)
    ...
    def test_failed_tasks_can_retry_failed_outputs_from_preview(self) -> None:
        task_routes = Path("codex_image/server/workspace_api.py").read_text(encoding="utf-8")
        script = self._frontend_script_source()
        task_actions = self._task_actions_source()
        styles = Path("codex_image/webui/static/styles.css").read_text(encoding="utf-8")

        self.assertIn('"/api/tasks/{task_id}/retry-failed"', task_routes)
        self.assertIn("tasks.resubmit_task", task_routes)
    ...
    def test_partial_failed_tasks_can_accept_successes_from_preview(self) -> None:
        ...
        self.assertIn('"/api/tasks/{task_id}/accept-successes"', task_routes)
